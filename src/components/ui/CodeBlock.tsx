...
function parseShell(code: string): ShellLine[] {
  ...
  /** Após blank: linhas com 2 espaços ou OUT| = saída (sem prompt). */
  ...
  for (const line of lines) {
    ...
    if (line.startsWith("OUT|")) {
      out.push({ kind: "raw", text: line.slice(4) });
      expectOutput = true;
      continuing = false;
      continue;
    }

    if (expectOutput && (line.startsWith("  ") || line.startsWith("	"))) {
      out.push({
        kind: "raw",
        text: line.startsWith("	") ? line.slice(1) : line.slice(2),
      });
      continuing = false;
      continue;
    }
 ...
 * real: prompt Debian, cores Tango, cursor piscando e comentários separados
      ...
      <div className="term-titlebar">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="flex items-center gap-2 text-xs font-mono min-w-0">
            {isShell ? (
              <TerminalSquare className="w-3.5 h-3.5 text-[#d70a53] shrink-0" />
            ) : (
          ...
          {shellLines.map((line, i) => {
            ...
            if (line.kind === "raw") {
              return (
                <div
                  key={i}
                  className={`term-line term-output whitespace-pre-wrap break-words ${C.dim}`}
                  style={{ animationDelay: delay(i) }}
                >
                  {line.text}
                </div>
