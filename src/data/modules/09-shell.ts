    ...
    practiceLabs: [
      {
        ...
        expected:
          "O whatis devolve uma linha por comando, com a seção do manual entre parênteses. O ls de /usr/share/doc traz changelog, copyright e, em muitos pacotes, uma pasta examples com arquivos de configuração comentados. Se o whatis não responder nada, rode sudo mandb para reconstruir o índice.",
    ...
    practiceLabs: [
      {
        ...
        expected:
          "A criação da sessão acontece em silêncio, e o list-panes imprime o comando em execução no painel: bash quando você acabou de criar, sleep enquanto o teste roda. Depois do detach, a sessão continua aparecendo em tmux ls — esse é o ponto do lab: fechar o terminal deixou de matar o trabalho.",
    ...
    practiceLabs: [
      {
        ...
        expected:
          "O git init imprime a mensagem de repositório vazio inicializado com o caminho, e o commit responde com a branch, o hash curto e a contagem de arquivos. Se o Git reclamar de identidade, configure user.name e user.email antes — sem isso ele se recusa a criar o commit, e nada foi perdido.",
    ...
    practiceLabs: [
      {
        ...
        expected:
          "A primeira linha impressa é o caminho do .venv e a segunda, a versão do requests. Se o prefixo apontar para /usr, você está no Python do sistema, não no ambiente virtual. Em Debian recente, instalar com pip fora do venv é recusado com o erro externally-managed-environment — e é justamente por isso que o venv existe.",
        verify:
    ...
    practiceLabs: [
      {
        ...
        expected:
          "Cada command -v imprime o caminho do binário encontrado e não imprime nada quando ele falta. Repare nos nomes usados pelo Debian: fd vira fdfind e bat vira batcat, por conflito com pacotes antigos. É por isso que script copiado da internet quebra aqui até você criar um alias ou um link.",
        verify:
